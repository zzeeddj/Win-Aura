...
* **🧘‍♂️ 禅模式 (Zen Mode)**
    * **`F8` 一键开启**：瞬间压暗屏幕背景，仅高亮当前活动窗口，提供极致的聚光灯专注体验。
    * **智能遮罩**：切换至桌面或开始菜单时自动隐藏遮罩，不影响系统操作。
...
* **🛡️ 智能过滤与防闪烁**
    ...
    * **Explorer 白名单：** 仅在文件资源管理器窗口显示，自动屏蔽属性弹窗和系统组件。
    * **进程锁 (PID-Lock)：** 独创的同步校验机制，彻底消除切换窗口时的“幽灵边框”闪烁问题。

* **⚡ 原生级性能**
    * **DWM 集成：** 直接调用 `dwmapi.dll` 获取视觉边界，完美贴合 Windows 11 圆角窗口。
...
RAM_SENSITIVITY_CAP = 10.0 # 内存满载阈值 (占系统总内存的百分比)

# 禅模式
ZEN_MODE_ALPHA = 180       # 遮罩透明度 (0-255)
ZEN_TOGGLE_KEY = 0x77      # 快捷键 F8 (win32con.VK_F8)
